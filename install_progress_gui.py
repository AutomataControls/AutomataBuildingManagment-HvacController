...
# Step 3: Set internet time using set_internet_time_rpi4.sh from the repository
REPO_DIR="/home/Automata/AutomataBuildingManagment-HvacController"
INTERNET_TIME_SCRIPT="$REPO_DIR/set_internet_time_rpi4.sh"
if [ -f "$INTERNET_TIME_SCRIPT" ]; then
    log "Setting internet time using $INTERNET_TIME_SCRIPT..."
    bash "$INTERNET_TIME_SCRIPT"
else
    log "Internet time script $INTERNET_TIME_SCRIPT not found. Skipping time setting."
fi
...
# Step 5: Clone the repository and set permissions
...
# Step 7: Copy the installation progress GUI script from the repo and set permissions
...
# Step 9: Run the installation progress GUI
